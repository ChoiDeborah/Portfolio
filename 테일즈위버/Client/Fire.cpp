...
HRESULT CFire::LateInit()
{
	m_wstrObjKey = L"Effect";
	m_wstrStateKey = L"BossSpell";
...
int CFire::Update()
{
	...
	else
	{
		...
		if (m_tFireAlarm.CheckAlarm())
		{
			CGameObject* pAttackEffect =
				CreateEffect(
					m_tInfo.vPos, D3DXVECTOR3{ float(rand() % 20 - 10), float(rand() % 20 - 10), 0.f },
					L"Effect", L"Cray"/*L"BossSpell"*/, true, OBJ_EFFECT, m_pTarget, m_pPlayer, FRAME_ONCE, false);
			...
			dynamic_cast<CAttackEffect*>(pAttackEffect)->SetColor(D3DCOLOR_ARGB(255, 255, 100, 100));
			dynamic_cast<CAttackEffect*>(pAttackEffect)->SetDebuff(DEBUFF_FREZE | DEBUFF_CONTINUOUS_DAMAGE);

			m_tFireAlarm.SetAlarm(0.01f, false);
		}
...
void CFire::Render()
{
	...
	int iRand = rand() % 9;
	m_pTexInfo = m_pTextureMgr->GetTexInfo(L"Effect", L"ElecShock", iRand);//L"FireRing", 0);
	...
	/*// 체인 머리
	m_pTexInfo = m_pTextureMgr->GetTexInfo(L"Effect", L"FireHead", 0);
	NULL_CHECK(m_pTexInfo);
...
void CFire::FrameOnceEnd()
{
	
}

void CFire::FrameEvent()
{
	
}

void CFire::AtarMoveEnd()
{
	
}

void CFire::AstarMoveSetState()
{
	
}

bool CFire::AstartExitEvent()
{
	return false;
}
