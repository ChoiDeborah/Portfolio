...
HRESULT CTextureMgr::ReadImagePath(const wstring & wstrFilePath)
{
	...
	CSceneMgr::GetInstance()->m_iMaxCount = int(_wtoi(szMaxCount));
	CSceneMgr::GetInstance()->m_iCount = 0;
	...
	while (!LoadFile.eof()) {
		...
		++CSceneMgr::GetInstance()->m_iCount;
	}

	CSceneMgr::GetInstance()->m_iMaxCount = 0;
	CSceneMgr::GetInstance()->m_iCount	= 0;
