...
HRESULT CDeviceMgr::InitDevice(MODE eMode)
{
	...
	// 렌더링 환경 설정.
	d3dpp.BackBufferWidth = CLIENTCX;	// 후면버퍼 가로 사이즈
	d3dpp.BackBufferHeight = CLIENTCY; // 후면버퍼 세로 사이즈
	...
	d3dpp.PresentationInterval = D3DPRESENT_INTERVAL_DEFAULT; // 수직동기화 킴 //D3DPRESENT_INTERVAL_IMMEDIATE; // 즉시 시연 한다.
	...
	tFontInfo.Width = 5;					// 폰트 가로 크기
	tFontInfo.Height = 11;					// 폰트 세로 크기
	tFontInfo.Weight = FW_LIGHT;			// 폰트 굵기
	tFontInfo.CharSet = HANGEUL_CHARSET;	// 지원 언어
	//lstrcpy(tFontInfo.FaceName, L"Consolas");	// 서체
	lstrcpy(tFontInfo.FaceName, L"휴먼명조");	// 서체
	...
	// 라인 Com객체 생성
	hr = D3DXCreateLine(m_pDevice, &m_pLine); // Line 생성
	FAILED_CHECK_MSG_RETURN(hr, L"D3DXCreateLine Failed", E_FAIL);
	m_pLine->SetWidth(5); // 라인의 굵기를 2로 설정
...
void CDeviceMgr::DrawFont(const wstring& wstrText, RECT * rt, UINT iOption, DWORD dwFontColor)
{
	m_pFont->DrawText(
		m_pSprite,				// 스프라이트 Com 객체
		wstrText.c_str(),		// 출력할 문자
		-1,						// 문자열 길이
		rt,						// 사각형 영역
		iOption,				// 사각형 영역 안에 그릴 때의 옵션
		dwFontColor				// 출력 색상
	);
}
